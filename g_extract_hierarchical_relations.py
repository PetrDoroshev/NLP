...
def mod_extract_hierarchical_relations(functional_triplets_path, hier_path, word_to_vec_model_path):
    json_triplets = shared_functions.load_dict_from_json(functional_triplets_path)
        ...
        for now_article_name in now_article:
            for now_triplet_js in now_article[now_article_name]:
                for now_triplet_id in now_triplet_js:
                    for now_triplet in now_triplet_js[now_triplet_id]:
                        all_triplets.append(now_triplet)


    model = Word2Vec.load(word_to_vec_model_path)

    hi_triplets = extract_hierarchical_relations(model, all_triplets)

    shared_functions.save_dict_as_json(hier_path, {"triplets": hi_triplets } )
...
def extract_hierarchical_relations(word2vec, functional_triplets):
    ...
    for cluster_id in range(num_clusters):
        ...
        if 1 < len(cluster_words) < max_cluster_size:
